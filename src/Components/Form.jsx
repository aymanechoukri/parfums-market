...
export default function Form(props) {
  ...
  async function toggelSubmit(e) {
    ...
    try {
      const res = await axios.post(`http://127.0.0.1:8000/api/${props.link}`, {
        name,
        email,
        password,
      ...
      if (res.status === 200) {
        window.location.pathname = `${props.local}`;
  ...
  return (
    <div className="flex justify-center items-center h-135 flex-col w-full">
      ...
      >
        <label htmlFor="full-name" className="text-md font-bold text-white mb-">
          Full name:
        </label>
        <input
          ref={nameRef}
        ...
        <label htmlFor="password" className="text-md font-bold text-white mb-">
          Password:
        </label>
        <input
          className="bg-gray-400/20 rounded-2xl py-2 px-5 border border-transparent
        ...
        />
        {password !== confirm &&
          accept && (<p className="text-red-600 text-sm">Not matching</p>)}
